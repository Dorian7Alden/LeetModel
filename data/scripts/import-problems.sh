...
import_problem "$data_dir/problem-01/problem.md" '[6001, 6101, 6102, 6201]'
# problem-02：可持续旅游业管理（MCM B题） -> 环境生态(6001) + 预测(6101) + 评价(6102) + 优化(6103) + 线性规划(6203)
import_problem "$data_dir/problem-02/problem.md" '[6001, 6101, 6102, 6103, 6203]'

echo "公开接口复核："
for problem_file in "$data_dir/problem-01/problem.md" "$data_dir/problem-02/problem.md"; do
  title="$(sed -n 's/^# //p' "$problem_file" | head -n 1)"
