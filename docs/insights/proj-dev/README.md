...
| `01-requirements-spec.md`     | 需求分析方法论 | 如何做需求分析：上下文 → 描述 → 边界 |
| `02-usecase-analysis-spec.md` | 用例分析方法论 | 如何识别用例：参与者 → 候选 → 三要素过滤 |
| `03-decomposition-spec.md`    | 模块拆分方法论 | 如何拆分模块：原则 → 步骤 → 分层 → 输出物 |
| `04-techstack-selection-spec.md` | 技术选型方法论 | 如何进行技术选型：原则 → 步骤 → 输出物 |
| `05-implementation-spec.md`   | 逐模块实现方法论 | 如何实现模块：原则 → 步骤 → 输出物 |
