...
> D-02 已确认两层网关边界：本服务保留内部契约、业务上下文、逻辑模型绑定、业务调度、统一错误和业务调用审计；new-api 唯一负责供应商密钥、渠道模型映射、渠道选择、渠道重试、额度和渠道健康。当前代码尚未切换，仍直连 DeepSeek 和 Kimi。完整边界见 [`new-api第三方网关集成.md`](../../02-架构设计/new-api第三方网关集成.md)。
