...
当前 Maven 运行模块、正式任务入口、异步工作流、数据库迁移和前端入口均已落地。公共创建入口使用 `GROUNDED_SUGGESTION_V3`；历史 `GROUNDED_SUGGESTION_V2` 与 `IMPROVEMENT_V1` 继续可读，代码和结果不被覆盖。

已确认设计但尚未实现的 `GROUNDED_SUGGESTION_V4` 将建议语义调整为确定性补缺、完整性增强和可选探索，所有长说明使用 Markdown，并由服务端补全真实原文和知识依据。V4 禁止把主观建模选择描述为唯一正确路径，也不承诺通过迭代建议自动产生获奖论文。
...
| 输入准备 | V3 已实现 | 自动晋级补齐 `PAPER_PARSE_V2` 与 `DEEP_EVIDENCE_REVIEW_V3` 证据资产 |
...
| 专业方向建议V4 | 已确认设计，尚未实现 | 三类建议语义、非唯一解措辞、原文证据、知识依据和统一 Markdown 展示 |
