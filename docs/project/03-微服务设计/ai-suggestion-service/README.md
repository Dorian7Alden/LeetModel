...
当前 Maven 运行模块、正式任务入口、异步工作流、数据库迁移、前端入口和管理端只读聚合均已落地。当前正式版本是 `IMPROVEMENT_V1`；新设计使用 `GROUNDED_SUGGESTION_V2`，不覆盖、不改写 V1 的历史行为。

> 分层定位：AI 业务能力层。本次只完成 V2 功能与架构设计，运行模块、接口、表和页面尚未实现。
...
| 功能 | 当前状态 | 目标设计 |
|------|----------|----------|
| 建议任务创建 | V1 已实现单任务幂等 | 任何已完成兼容评审的论文版本可由有权用户手动创建，同版本支持多次生成 |
| 输入准备 | V1 读取题目、PDF 文本和评审 JSON | V2 优先读取评审锁定的解析产物；历史数据补齐解析并准备可追溯评审依据，再固化全部版本快照 |
| 知识上下文 | V1 未实现 | 调用独立知识检索服务，支持向量 RAG、受控 AI 选文与混合检索版本 |
| 证据化建议 | V1 只校验可选页码 | V2 要求可定位的论文依据、稳定评审发现和可追溯知识引用 |
| 任务进度与重试 | V1 已实现 | V2 区分“同快照重试”与“用户新的生成” |
| 版本目录 | 尚未实现 | 发布 `GROUNDED_SUGGESTION_V2`，并保留 `IMPROVEMENT_V1` 可读 |
