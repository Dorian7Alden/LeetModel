...
> 实现状态：S7 已通过 V2 至 V6 完成通用模型、attempt 幂等、控制审计、原始指标和数据集版本快照迁移；当前事实以 Flyway、实体和运行测试为准。兼容退出门槛见 [迁移回归与兼容退出.md](迁移回归与兼容退出.md)。
...
SUGGESTION 已在 S9 完成隔离实验与评测闭环打通。`ai-suggestion-service` 提供 `/internal/suggestions/feature-definition` 与 `/internal/suggestions/experiments` 隔离执行契约（透传 `evaluationTaskId`，执行工作流但不入正式业务库）；样本采用 `sampleType=SUBMISSION_REFERENCE` 与 `payloadSchemaVersion=SUGGESTION_SUBMISSION_V1`（引用 `submissionId`）；候选工作流支持 `IMPROVEMENT_V1`（默认模型配置 `MODEL_CFG_SUGGESTION_TEXT_0001`）与 `GROUNDED_SUGGESTION_V2`。
...
7. 应用层继续读取新字段与旧 REVIEW 字段，直至 [迁移回归与兼容退出.md](迁移回归与兼容退出.md) 的五项退出门槛全部满足；所有新写入只走通用模型。
