...
> 设计状态：S7 通用评价平台及迁移回归已落地，包括版本化样本、REVIEW/ASSISTANT Runner、批次控制、`METRIC_SET_V1/V2` 原始指标和同口径比较门禁；S8 已落地归一化规则、评价服务自有的版本化权重方案、首版“版本选择指数”快照与不覆盖历史的方案重算。旧版 `overallScore` 仍只作为兼容字段。
...
4. 当前调度器每次领取一个待运行槽位，按 feature 选择 REVIEW 或 ASSISTANT Runner 调用 owner 的隔离实验接口。实验不创建正式评审任务、客服会话或消息。RocketMQ 目标设计会用独立 `evaluation-task-v1` 的 `EVALUATION_SLOT_READY` 消息唤醒已落库槽位，完整实验仍由本服务低并发 Worker 执行。
...
- 目标设计中生产和消费后台评价就绪消息；在线核心任务积压时暂停领取，不用消费失败实现限速。
