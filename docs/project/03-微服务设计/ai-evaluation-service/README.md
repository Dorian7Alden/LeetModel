...
> 设计状态：通用领域模型、版本化样本、REVIEW/ASSISTANT Runner、创建前规模预估和 `GENERAL_AI_METRICS_V1` 原始指标快照已经落地；旧版 `overallScore` 仍只作为兼容字段，不是正式需求中的质量分。后续任务继续完善可验证质量指标、口径版本、权重快照和“版本选择指数”。
...
- 真实 MySQL 8 已验证 Flyway V1→V2→V3 连续迁移、通用四表回填以及 attempt 幂等唯一索引。
