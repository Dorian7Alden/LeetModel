...
#### 3. 评委认知框架与引导思考链对齐
全套评审提示词矩阵全面对齐 [数模专家评委认知框架与双向提示词设计规范](../../数模专家评委认知框架与双向提示词设计规范.md)，注入评委“审什么、疑什么、盼什么”的三层审视模型，强制排查数模 8 大失分陷阱（套模板、无量纲、假收敛、缺基准、弱假设、伪检验等），并采用引导性思考链（Thought Process）杜绝死板八股教条。
