...
> 设计状态：已完成实现并经验收。字段名是实现依据草案，最终以 DTO、Flyway 和测试为准。
...
- 原文块按锁定解析产物完整生成 Markdown，不由服务端静默插入省略号或节选标记；
  前端不得对说明正文和原文依据做二次截断。
- 保存 `contentHash`，证明摘录来自不可变解析块。
- `TABLE` 等原始 HTML 中的行内 `$...$` 由统一 Markdown 渲染器转换为 KaTeX，
  避免表格单元格把公式定界符原样展示。
...
- 原文依据必须来自当前评审锁定的 `parseArtifactId`，不能从前端当前选择的解析版本或其他历史结果补写。
