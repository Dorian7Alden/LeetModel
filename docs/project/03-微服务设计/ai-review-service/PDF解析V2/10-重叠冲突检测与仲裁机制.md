...
BEGIN
    ...
    ELSE
        ...
        -- 降级容错：若仲裁 AI 调用失败，选择信息量更完整的非空版本
        IF finalTextBlocks IS NULL THEN
            finalTextBlocks <- SelectRicherNonEmptyVersion(chunkA.textBlocks, chunkB.textBlocks);
        END IF
    END IF
...
模型偶发会把 `heading/formula/table/figure/code/references` 等可选对象输出为布尔值。
响应解析器统一把布尔值规范化为 `null`，窗口缓存复用和实时模型响应必须经过同一清洗路径。
仲裁异常时严禁固定选择前序窗口；当 A 为空而 B 有内容时，必须保留 B，避免整页丢失。
