...
#### 阶段二：双页滑窗调度与前文参考注入
1. **页数边界判定与滑窗序列生成**：
   - 若 $N = 0$（空文件）：阶段一直接阻断，记录任务为 `FAILED`，抛出无页面异常；
   - 若 $N = 1$（单页文档）：特化为单次窗口调用 $W_1 = (\text{Page } 1)$，仅输入第 1 页单图，跳过前文注入与重叠仲裁，直接输出全篇产物；
   - 若 $N \ge 2$（常规多页）：全篇页面统一视为普通正文页，固定步长为 1 生成滑动窗口 $(1, 2) \rightarrow (2, 3) \rightarrow ... \rightarrow (N-1, N)$，不对摘要页或目录页做任何跳过或分支处理，总窗口数为 $N-1$。
