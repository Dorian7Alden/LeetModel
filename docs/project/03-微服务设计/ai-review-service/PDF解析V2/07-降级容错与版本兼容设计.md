...
- 最终内容块不能为空；
- 物理首页和末页必须至少包含一个内容块；
- 缺失页数不得超过总页数的 5%，且至少允许 1 页非正文空白；
- 历史 `SUCCESS` 或 `PARTIAL_SUCCESS` 产物复用前执行同一校验；
- 扫描型 PDF 的 PDFBox 降级为空时，不得以 `PARTIAL_SUCCESS` 进入评审。

该质量门用于防止“解析流程形式完成，但论文证据实际缺失”的产物污染评分。
