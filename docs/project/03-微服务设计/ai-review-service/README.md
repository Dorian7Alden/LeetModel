...
当前 `BASIC_REVIEW_V1` 已实现；目标版本 `EVIDENCE_REVIEW_V2` 新增强制 PDF 解析、完整题目要求、评分说明、要求覆盖状态和可定位评审发现。V2 是新工作流，不覆盖 V1 的代码、数据或历史展示。
...
flowchart LR
    ...
    subgraph review["ai-review-service 版本化评审"]
        ...
        taskApi --> scheduler --> inputPreparation
...
ai-review-service 独占 `lm_review` 数据库，拥有评审版本、任务、最终评分、版本专属结果、解析工作流版本和解析产物。建议服务优先读取某次已完成评审锁定的解析产物；对历史 V1 论文可按明确版本请求补齐。建议服务不自行解析、不选择“当前最新”解析结果。
...
| 证据化评审 | 已完成设计 | 新增 `EVIDENCE_REVIEW_V2`，输出评分说明、要求覆盖、优点与问题发现 |
| PDF 解析 | V1 不使用 | V2 强制使用版本化解析产物；解析失败不静默绕过 |
| 评审结果校验 | V1 校验结构和总分范围 | V2 服务端汇总总分并校验全部引用 |
| 评审结果查询 | 已实现基础查询 | 根据版本返回可展示的评分说明和发现，历史 V1 不伪造 V2 字段 |
| 隔离评审 | 已有 REVIEW 实验入口 | V2 发布后以新工作流版本进入评价目录 |
...
| [AI评审/V2证据化评审/README.md](AI评审/V2证据化评审/README.md) | 目标 V2 评分说明、要求覆盖、可定位发现与解析依赖 |
