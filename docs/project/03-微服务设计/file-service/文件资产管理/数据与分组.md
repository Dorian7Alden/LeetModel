...
> 实施状态：`file_asset` 已落地并补充内容摘要与访问级别字段；`file_binding` 引用投影与绑定/解绑事件消费已实现；`file_upload_session` 已落地，用于预签名分片直传会话。
...
第一阶段已保存 bucketName、objectKey、originalName、contentType、fileSize、namespaceCode、groupPath、sourceType、lifecycleStatus、creatorId 和删除重试字段。下表同时包含后续迁移需要补齐的目标字段。
...
目标 objectKey 由文件资产链路生成，完整建议结构为：

```text
{namespace}/{groupPath}/{yyyy}/{MM}/{uuid}.{extension}
```

第一阶段代理上传实际使用 `{namespace}/{groupPath}/{uuid}.{extension}`，暂不增加年月层级。groupPath 为空时省略对应路径段。原始文件名只保存在元数据中；对象身份使用 UUID，避免同名覆盖和路径遍历。
