...
> 设计状态：公共审计契约、专用 RocketMQ 资源、audit-service Maven 模块、`lm_audit` Flyway V1、严格消费者/幂等归档、受信内部只读查询、主要领域/治理生产者和管理端审计页已建立。本文确定服务边界；跨服务完整设计见 [操作审计架构](../../02-架构设计/操作审计架构.md)。
...
| 领域生产者（内容/提交） | 已实现基础 | problem-service 与 submission-service 只追加目录允许的最小摘要，业务事实与 Outbox 同事务 |
| AI、消息与派生治理生产者 | 已实现基础 | AI 队列/评价、生产配置、Consumer 运维、Outbox 重放和排行重建追加治理摘要；调用计量仍留在领域事实表 |
| 完整性与管道监测 | 基础已实现 | 输出 Inbox 处理中、非法消息、失败、DLQ 及超 deadline 未完成操作的低基数指标 |
| 保留、归档与备份 | 已实现配置基线 | 在线保留、归档开关、备份要求和策略版本外置配置；自动删除不启用 |
| 统一管理审计页 | 已实现基础 | admin-service 角色保护的只读代理和前端筛选/阶段详情/Trace 复制；中央或来源不可用显式提示，不提供修改、删除和默认批量导出 |
| 审计导出治理 | 目标 | 后续若开放导出，导出自身必须受权限控制并产生审计，不提供默认批量下载 |
