...
> 分层定位：AI 业务能力层。MVP 会话、关键词触发的题目候选预取和常规向量 RAG V1 已落地；标准受控工具调用已形成待确认设计草案但尚未实现。长期记忆、开放式 Agent、自主写操作、语音和多模态会话仍不在当前范围。
...
| 受控工具调用 | 设计草案待确认 | 使用标准工具协议调用题目查询、题目推荐和专用知识讲解工具 |
