...
#### 3. `delta`（文本增量片段）
大模型生成文本片段，驱动前端打字机平滑流式吐字。该事件必须在模型产生对应文本后立即逐层透传，不得先等待完整回答，再把完整内容人为切片伪装成流式输出：
```text
event: delta
...
1. **打字机与工具条折叠**：
   ...
   - 收到首个 `delta` 后立即将内容加入同一打字机队列；后续 `delta` 只追加队尾，不重置正文；
   - 打字机以固定时间间隔逐个 Unicode 字符消费，不因回答长度或队列积压提速；
   - 收到 `message_end` 时以 `fullContent` 校准尚未收到的尾部内容，继续按原速度排空后再解除发送锁。
3. **后端透传边界**：
   - `ai-gateway-service` 逐帧解析 new-api 的文本 `delta`，并聚合可能跨帧拆分的 `tool_calls` 供工具循环继续执行；
   - `ai-assistant-service` 的普通回答和工具编排最终回答都直接转发模型文本增量；工具规划阶段没有可见文本时允许只显示工具状态；
   - 终止型知识工具因回答还需执行范围标记转换、清洗和长度限制，当前仍在校验完成后作为一个整体进入同一前端打字队列，不能透传未经校验的原始文本。
2. **断连容灾与自动回退**：
