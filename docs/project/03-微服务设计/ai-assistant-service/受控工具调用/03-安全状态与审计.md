...
> 实现状态：V5 Flyway 已创建 `assistant_tool_call`，`RECEIVED → RUNNING → COMPLETED|FAILED|TIMED_OUT` 与 `RECEIVED → REJECTED` 条件迁移、参数/结果快照和 AI 调用关联已落地。消息级 `toolsetVersion` 与 `attemptCount` 快照随 S11-05 生产工具工作流发布。
