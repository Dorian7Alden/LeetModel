...
- 维护可直接渲染的 Markdown 题面以及题目附件的归属、说明和顺序。当前同时保存对象存储路由，目标迁移后保存稳定 fileId。
- 维护题目标签和题目标签关系。
...
- 不维护队伍、成员、提交和评审结果。
- 题目详情可返回由维护者编辑的 `solutionHint` 精短解题提示；前端不从 Markdown 自动推导提示或任务结构。
- 不因为题目被练习就复制其他服务的主数据。
...
problem-service 独占 `lm_problem` 数据库，预置赛事、题目、Markdown 题面、附件业务元数据、标签和题目标签关系以这里的数据为事实源。题目必须归属一个赛事，附件只属于一道题目。当前附件二进制内容保存在 MinIO，数据库保存对象路径和必要元数据。目标 file-service 建成后，题目附件关系仍由 problem-service 拥有，文件技术元数据和物理生命周期迁移到 file-service，problem-service 改为保存稳定 fileId 并发布绑定或解绑事件。
...
| 附件管理 | 维护一道题目的零个或多个附件及其说明；目标通过 fileId 关联 file-service 文件资产 |
...
| 题目收藏 | 记录用户对题目的收藏关联与点击时刻，支持收藏增删、状态查询与按收藏时间升序题单分页 |
