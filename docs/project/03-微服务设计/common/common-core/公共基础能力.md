...
- `Exception` → 兜底，返回 `SYSTEM_ERROR`；日志仅记录异常类型、固定失败类别和有界代码位置，不复制异常消息
...
- **SkyWalkingLogReporterAppender / SkyWalkingLogReporterMetrics**：通过 OAP 原生 `/v3/logs` 单线程批量上报；业务线程仅向有界队列投递，短超时、有限重试、低级别优先丢弃，所有失败/丢弃/恢复均可由 Prometheus 观察
