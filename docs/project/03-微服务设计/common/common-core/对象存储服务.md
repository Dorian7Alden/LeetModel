...
为了保持全模块统一的技术分层规范，`MinioConfig` 与 `MinioProperties` 统一定位在 `com.leetmodel.common.core.config` 包中，与 Jackson、MyBatis-Plus 配置保持一致；`storage` 目录仅保留服务接口 `StorageService` 与具体实现。
