...
| [common-messaging/](common-messaging/) | RocketMQ 消息信封、事务 Outbox、Inbox 幂等、租约 Relay、指标和测试替身 |
| [common-ai/](common-ai/) | AI 网关客户端、统一调用契约和测试替身 |

`common-ai` 已建立为 Maven 公共模块，提供统一请求与响应契约、AI 用量模型和调用 AI 网关的客户端。

`common-messaging` 已在 MQ1 建立公共契约和可靠传递基础能力。Outbox 与 Inbox 表仍位于各数据所有者数据库，具体 Topic、Tag、Payload、任务状态与补偿规则仍由业务服务拥有。完整边界见 [RocketMQ 消息队列](../../02-架构设计/RocketMQ消息队列.md)。
