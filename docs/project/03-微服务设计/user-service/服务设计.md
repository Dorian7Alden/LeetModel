...
- **Flyway**：数据库版本管理，V1=user 表，V2=RBAC 四表 + 初始数据，V3=联调用户数据，V4=扩展头像字段长度，V5=将头像字段重命名为 `avatar_path`
...
- **MinIO 头像存储**：首次上传时由存储组件检查并创建 Bucket，数据库只保存与域名无关的对象路径，接口响应时再生成头像访问 URL
