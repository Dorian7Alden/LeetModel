## 用户服务

> 创建日期：2026-07-26

---

### 一、职责

用户服务（端口 8081）：注册登录、RBAC 五表权限管理、为 common-security 提供角色权限数据。

---

### 二、包结构

采用标准分层结构：controller（对外 API + 内部 Feign 实现）、service（认证/用户/角色）、mapper（MyBatis-Plus BaseMapper）、entity（5 个实体）、dto（请求/响应对象）、enums（模块错误码）。数据库迁移脚本位于 `resources/db/migration/`。

---

### 三、RBAC 五表

```
user (1) ── (N) user_role (N) ── (1) role (1) ── (N) role_permission (N) ── (1) permission
```

#### 初始数据

| 角色 | 编码 | 拥有权限 |
|------|------|---------|
| 管理员 | admin | user:read, user:update, submission:create, suggestion:create |
| VIP | vip | submission:create, suggestion:create |
| 普通用户 | user | submission:create |

---

### 四、API 接口

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/auth/register` | 注册 | 无需 |
| POST | `/api/auth/login` | 登录并返回 JWT、角色和权限 | 无需 |
| POST | `/api/auth/logout` | 登出 → Token 加入黑名单 | 需登录 |
| GET | `/api/users/me` | 查询当前用户资料 | 需登录 |
| PUT | `/api/users/me` | 更新当前用户资料 | 需登录 |
| PUT | `/api/users/me/password` | 修改当前用户密码 | 需登录 |
| POST | `/api/users/me/avatar` | 上传当前用户头像 | 需登录 |
| GET | `/api/users/me/authorization` | 查询当前用户角色和权限 | 需登录 |
| GET | `/internal/users/{userId}/roles` | 查询角色权限（内部 Feign） | 内部 |
| GET | `/internal/admin/permissions` | 查询权限列表 | 内部 |
| GET | `/internal/admin/permissions/{permissionId}` | 查询权限详情 | 内部 |
| POST | `/internal/admin/permissions` | 创建权限 | 内部 |
| PUT | `/internal/admin/permissions/{permissionId}` | 更新权限 | 内部 |
| DELETE | `/internal/admin/permissions/{permissionId}` | 删除权限 | 内部 |
| GET | `/internal/admin/roles/{roleId}/permissions` | 查询角色权限 | 内部 |
| PUT | `/internal/admin/roles/{roleId}/permissions` | 全量更新角色权限 | 内部 |

---

### 五、关键技术点

- **Flyway**：数据库版本管理，V1=user 表，V2=RBAC 四表 + 初始数据，V3=联调用户数据，V4=扩展头像字段长度，V5=将头像字段重命名为 `avatar_path`，V6=补充演示用户，V7=可靠消息 Outbox/Inbox 表，V8=头像改为 `avatar_file_id` 并保留外链 `avatar_url`
- **BCrypt**：密码加密，Spring Security 内置
- **注册时自动分配 user 角色**：user_role 表插入 `(userId, 3)`
- **common-security 依赖 common-api 声明接口，user 服务实现接口**：`InternalUserController` 路径匹配 `UserFeignClient` 的声明
- **头像文件资产**：上传的头像按 `USER_AVATAR` 用途登记到 file-service，数据库只保存稳定 `avatar_file_id`，响应时由 file-service 生成有界时效的访问地址；演示与遗留外链头像继续保存在 `avatar_url`，用户停用或更换头像时解绑旧引用

---

### 六、RBAC 职责边界

- user-service 拥有 RBAC 五表，负责角色权限数据、业务校验和本地事务。
- admin-service 提供管理端入口并执行管理员鉴权，不直接访问 RBAC 数据库。
- common-api 只定义跨服务请求、响应和 Feign 契约。
- common-security 消费用户角色权限数据并执行鉴权，不负责授权数据管理。

---

### 七、权限管理决策

- 权限编码采用 `资源:动作` 格式并保持全局唯一。
- 已被角色引用的权限禁止直接删除，管理员必须先解除角色权限关联。
- 删除保护用于避免误操作造成多个角色同时失去权限。
- 权限数据当前不使用业务缓存，优先保证变更后的鉴权结果一致。
- 角色权限更新采用全量替换语义，在 user-service 本地事务中完成。
- 用户角色和角色权限写入前必须去重并验证所有目标 ID 存在。
- `admin`、`vip`、`user` 是系统预设角色，不允许删除或修改编码。
- 注册通过 `user` 编码获取默认角色，不依赖固定数据库 ID。
- 用户管理查询使用 `user:read`，用户状态和角色修改使用 `user:update`。
- 管理端用户分页支持用户名、昵称、邮箱关键字及账号状态、角色组合筛选；外层用户查询继续应用逻辑删除条件。
- 用户、角色和权限管理契约中的 Long 标识按 JSON 字符串输出，计数仍保持数值，避免浏览器对雪花 ID 发生精度舍入。
- 角色列表返回当前有效用户数和权限数，权限列表返回引用角色数。有效用户数在数据库中关联 `user.deleted = 0` 后聚合，历史逻辑删除用户的中间表记录不计入管理端影响范围。
