...
本文描述的是知识库从本地目录迁移至对象存储的目标架构；当前代码仍处于迁移前阶段，边界如下：

- `knowledge-retrieval-service` 通过 `KNOWLEDGE_BASE_PATH` 读取知识正文，默认路径为仓库内的 `rag_kb/`，Manifest、目录树和检索索引均由该事实源构建；
- 管理端知识库 ZIP 导出会从该本地事实源打包，ZIP 导入目前只做内存解压、`README.yaml` 校验和 Manifest 版本计算，不会写入 MinIO，也不会写入 `file-service` 的资产表；
- `file-service` 当前只负责通用文件资产（例如图标、宣传图、题目附件等）的 MinIO 桶、分组和生命周期管理，不能把 `lm_file` 当作知识库正文事实源；
- 后续迁移到 MinIO 时，需要在知识库服务内增加对象存储适配、知识文档元数据持久化和导入后的发布/索引流程，并保持 `file-service` 与知识库服务的数据所有权边界不变。
