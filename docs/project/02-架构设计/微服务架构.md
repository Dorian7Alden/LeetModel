...
│   ├── ai-evaluation-service/   ← AI 质量评价与版本对比
...
| **ai-evaluation** | 8091 | AI 评审固定数据集、隔离实验、MVP 指标与版本对比 | 质量评价启动、失败重试与结果查询 | lm_ai_evaluation |
...
业务 Prompt 和生成工作流程留在各自业务服务。AI 网关只提供模型调用治理，不吸收业务生成或质量评价逻辑。AI 质量评价规则、固定样本、指标归一化和综合得分由 ai-evaluation-service 统一管理。AI 网关提供调用、用量、成本、耗时和状态数据，管理后台负责聚合展示。
...
AI 质量评价服务独占 lm_ai_evaluation，负责评价任务、样本集、评价指标、归一化结果和综合得分。当前虽然主要评价 AI 论文评审，仍提前拆分为独立服务，以保持 AI 生成与 AI 质量评价的结构边界，并支持后续论文改善建议等评价场景。
