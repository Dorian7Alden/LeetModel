...
- `docs/project/03-微服务设计/ai-suggestion-service/README.md` 仍声称当前服务尚无 Maven 运行模块，整张流程图都是目标设计。
...
`ai-suggestion-service` 是真实 Maven 运行模块。D-01 固定该事实；S6-01 再盘点它的正式入口、数据所有权、版本来源和未完成缺口。在 S6-01 之前不根据旧 README 重建模块。
