...
`ai-evaluation-service` 负责评价 AI 业务能力及其版本，而不代替业务服务生成用户结果。当前已支持 `REVIEW`、`ASSISTANT` 与 `SUGGESTION` 三类评价目标，并实现指标集、资源指标引用、归一化、权重方案和版本选择指数；其中重复评分稳定性只适用于评审类评价，其余功能按原始指标中的耗时、Token、费用、成功率与确定性质量指标衡量。
...
- ai-evaluation-service 已支持评审、客服与建议三类评价：隔离实验 Runner、稳定性统计、指标集、权重方案和版本选择指数均已落地，建议评价复用提交样本与隔离建议实验接口。
