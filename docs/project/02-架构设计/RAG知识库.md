...
> 当前设计整体暂缓。现有知识库内容已归档到 `legacy/knowledge-base/`，当前开发阶段不读取、不维护、不实现本文方案。本文仅保留历史设计决策，不代表当前任务或近期路线。只有用户重新确认并建立独立任务卡后，才允许恢复设计校准与实现。
...
原设计中，知识库以仓库根目录 `knowledge-base/` 作为运行时目录。该目录当前已归档为 `legacy/knowledge-base/`，归档路径不作为运行时知识库路径。未来恢复 RAG 时必须重新确认实际目录和加载方式。
