...
| **项目结构** | Maven 多模块 | 1 个父 POM、6 个公共 Jar 和 15 个可独立启动的服务 |
...
| **令牌** | JWT + Redis 黑名单 | - | JWT 无状态签发 + Redis 黑名单实现登出即时失效；已实现，按 Token 指纹写入安全状态 Redis 并设置剩余有效期 TTL |

**JWT 无状态性与黑名单的设计权衡**：

JWT 的设计理念是"无状态"——服务端不存 Session，靠签名自包含验证。但纯无状态 JWT 存在一个致命缺陷：**无法主动失效**。签发的 Token 在过期前始终有效，无法实现"踢人下线"或"强制登出"。

本项目通过 Redis 维护 Token 黑名单来补上这块能力：登出时按 Token 的 SHA-256 指纹写入安全状态 Redis（TTL 取剩余有效期），网关与服务侧校验时先查黑名单，命中即拒绝。代价是每次请求多一次 Redis 查询，换来登出即时生效的确定性。
...
**面试可讲点**：
- 为什么选 Sa-Token 而不是手写 Security 配置？—— 开发效率与可维护性的权衡，Sa-Token 的 `@SaCheckRole` 注解与 JWT 无状态模式开箱可用
- JWT 无状态 vs 黑名单矛盾吗？—— 不矛盾，这是"无状态签发 + 有状态增强"的混合模式：签发时保持无状态优势（不查库），校验时通过黑名单补齐主动失效能力；Sa-Token 1.38 的 JWT 无状态模式自身不提供黑名单，这部分由项目自建

#### 5.4 当前容量与故障治理

系统构建分层流量治理与容错防线，明确 Sentinel、RocketMQ 与自研 AI 调度器的职责边界：

- 网关层引入 Sentinel Gateway Adapter，针对路由与 API 分组实施 QPS 流量整形，提供统一 429 降级信封与 TraceId 关联；内部 Feign 同步调用基于 Sentinel 熔断器实施慢调用/异常容错，完整设计见 [流量治理与服务容错.md](流量治理与服务容错.md)。
...
严禁让 Sentinel 侵入长耗时大模型调度（3s~30s）或 RocketMQ 异步消费端（避免滑动窗口抛错引发死循环重试）。
...
| 组件 | 选型 | 版本 | 选型理由|
|------|------|------|---------|
| **搜索引擎** | Elasticsearch | 8.14.3 | RAG 向量索引与 KNN 检索，以及题库标题/题面全文检索；题库事实源仍为 MySQL |
...
| problem-service | 公开题库关键词检索，命中标题与题面并按相关度排序 | text + `cjk` 分词 + BM25 倒排检索 |

题库全文检索的索引结构、同步与降级规则见 [公开题库全文检索](../03-微服务设计/problem-service/公开题库/全文检索.md)。
...
当前 15 个服务已统一接入 Micrometer、Prometheus Registry、受保护的 Actuator 抓取、六类 Grafana 看板和 22 条主动告警，但仍没有引入 Micrometer Tracing bridge。跨服务关联依靠项目自定义 `X-Trace-Id`、MDC、Feign 拦截器、消息信封和数据库 `traceId` 字段。
...
| **业务服务启动脚本** | `start-mvp.sh` 构建并以本地 JDK 启动 15 个服务；当前仓库没有每服务 Dockerfile |
...
| Spring Cloud Alibaba | 2023.0.1.0 | 对应 Spring Cloud 2023.0.x，提供 Nacos 与 Sentinel 服务保护集成 |
| 服务保护/熔断限流 | Sentinel 1.8.6 | 网关层与 Feign 客户端防雪崩；与长任务 MQ 削峰和 AI 网关调度严格解耦 |
...
| 6 | 分层容量治理 + Sentinel 防护 | 纯网关限流 / Resilience4j / 无熔断 | 边缘网关防刷与 Feign 慢调用熔断交由 Sentinel；长耗时 AI 调度与异步削峰保持独立治理 |
