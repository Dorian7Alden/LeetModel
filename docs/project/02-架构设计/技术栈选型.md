...
| 生产者 | 消息类型 | 消费者 | 说明|
|--------|---------|--------|------|
| submission-service | 评审任务 | ai-review-service | 作品提交后触发 AI 评审|
| ai-review-service | 排名更新事件 | ranking-service | 评分完成后更新排行榜|
| suggestion (API) | 建议任务 | suggestion (worker) | VIP 用户请求改进建议，异步处理 AI 调用|
| ai-review-service / ai-suggestion-service | 业务事件 | admin-service | 操作日志采集|
