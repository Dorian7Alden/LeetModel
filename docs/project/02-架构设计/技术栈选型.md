...
flowchart TB
    ...
    subgraph AI["AI 集成层"]
        ...
        AI_GATEWAY["AI 网关<br/>模型路由 + 调用治理"]
        LC4J["LangChain4j<br/>供应商适配 + 结构化输出"]
...
| **项目结构** | Maven 多模块 | 1 个父 POM、4 个公共 Jar、2 个网关服务和 9 个业务微服务|
...
| 组件 | 选型 | 版本 | 选型理由|
|------|------|------|---------|
| **消息队列** | Apache RocketMQ | Broker 5.5.1，Spring 2.3.3 目标基线 | 以 NORMAL Topic 承载跨服务任务与业务事件，使用事务 Outbox、消费 Inbox、Broker 重试和 DLQ 保证可恢复传递；完整设计见 [RocketMQ 消息队列](RocketMQ消息队列.md) |
...
| AI 网关 | 供应商维度限流 + 熔断降级 | 汇总全部业务服务的模型流量，保护供应商配额并触发主备切换|
| ai-review-service | 评审任务并发控制 | 控制评审任务吞吐，不重复实现供应商级限流|
| suggestion 建议服务 | 业务入口限流 | VIP 功能按业务配额控制请求量|
...
|------|------|-----------------|
| assistant | 多轮对话、选题推荐和客服 RAG V1 | ChatMemory（会话记忆）+ @Tool（查题目库）+ RAG 检索组件 |
| ai-review-service | 多维度评审打分 | 结构化输出（Schema）+ Prompt 模板|
| suggestion | 论文改进建议 | Prompt 模板与结构化输出；不是 RAG V1 首个消费者 |

业务服务通过 `common-ai` 调用 AI 网关，不直接集成模型供应商。工作流或实验配置在任务创建时锁定每个步骤使用的精确模型和参数；AI 网关负责校验固定绑定，并在同一模型的多个账号或端点之间调度执行，不跨模型动态切换。
...
| ai-review-service | RAG 知识库向量检索（评审标准、评分细则） | dense_vector + KNN|
...
| RocketMQ | Broker 5.5.1，Spring 2.3.3 目标基线 | 实施任务先用 JDK 17 与 Spring Boot 3 做真实协议、重启恢复和消息类型兼容验证 |
...
| 3 | RocketMQ 5.5.1 目标基线 | RabbitMQ / Kafka | Java 生态与消费组、重试、DLQ 和多消息类型能力完整；项目用 Outbox/Inbox 明确承担端到端可靠性 |
