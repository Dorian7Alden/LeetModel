...
当前 13 个服务已统一接入 Micrometer、Prometheus Registry、受保护的 Actuator 抓取、六类 Grafana 看板和 22 条主动告警，但仍没有引入 Micrometer Tracing bridge。跨服务关联依靠项目自定义 `X-Trace-Id`、MDC、Feign 拦截器、消息信封和数据库 `traceId` 字段。
