...
当前项目使用 Micrometer 指标接口记录缓存、消息和领域任务的低基数指标，但尚未接入 Prometheus Registry 和完整采集后端，也没有引入 Micrometer Tracing bridge。跨服务关联依靠项目自定义 `X-Trace-Id`、MDC、Feign 拦截器、消息信封和数据库 `traceId` 字段。
...
| 能力 | 当前基线 | 目标选型 | 边界 |
|------|----------|----------|------|
| Metrics | Micrometer 局部埋点 | Micrometer + Prometheus | 低基数指标和告警权威来源，不保存业务 ID |
...
| 看板与告警 | Grafana 六类看板与 Alertmanager 基础路由已落地；生产告警待完成 | Grafana + Alertmanager | Prometheus 规则为唯一告警计算与通知入口 |
