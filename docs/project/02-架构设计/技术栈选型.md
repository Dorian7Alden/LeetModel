...
| review 评审服务 | 消费者并发控制 | 控制评审任务吞吐，不重复实现供应商级限流|
...
业务服务通过 `common-ai` 调用 AI 网关，不直接集成模型供应商。供应商、模型、参数和主备策略由 AI 网关按业务场景动态路由。
...
| ai-review 服务 | RAG 知识库向量检索（评审标准、评分细则） | dense_vector + KNN|
