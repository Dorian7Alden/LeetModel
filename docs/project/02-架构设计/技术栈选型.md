...
flowchart TB
    ...
    subgraph Toolchain["工具链 & 部署"]
        ...
        subgraph Toolchain2["&nbsp;"]
            ...
            MICROMETER["Micrometer Metrics + 显式 traceId<br/>可观测"]
...
| **项目结构** | Maven 多模块 | 1 个父 POM、6 个公共 Jar 和 13 个可独立启动的服务 |
...
项目使用 Micrometer 指标接口记录缓存、消息和领域任务的低基数指标，但没有引入 Micrometer Tracing bridge。跨服务追踪使用项目自定义 `X-Trace-Id`、MDC、Feign 拦截器、消息信封和数据库 traceId 字段串联。

| 组件 | 选型 | 版本 | 选型理由|
|------|------|------|---------|
| **Metrics** | Micrometer | 随 Spring Boot 3.x | Spring Boot 3.x Actuator 默认 metrics 门面|
| **链路追踪** | 自定义 traceId 传播 | 项目实现 | 网关生成或复用 `X-Trace-Id`，HTTP、Feign、RocketMQ 和领域任务继续传播并持久化 |
...
**面试可讲点**：微服务下怎么排查跨服务调用问题？—— 网关通过 `X-Trace-Id` 建立调用标识，Feign 拦截器、RocketMQ 消息信封和领域任务继续传播。管理端可以按 traceId 聚合 Outbox、Inbox 和 AI 调用事实。当前方案轻量且已覆盖业务主链，但没有 span、拓扑和自动埋点，不能等同于完整 OpenTelemetry 链路追踪。
...
| **业务服务启动脚本** | `start-mvp.sh` 构建并以本地 JDK 启动 13 个服务；当前仓库没有每服务 Dockerfile |
...
| 可观测性 | Micrometer Metrics + 自定义 traceId | 当前没有 Micrometer Tracing bridge |
