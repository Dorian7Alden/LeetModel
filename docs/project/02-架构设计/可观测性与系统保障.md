...
> 设计状态：目标设计已确认，已建立可重复的本地 SkyWalking/BanyanDB/Prometheus/Grafana/Alertmanager 环境，详见 [可观测性组件基线](可观测性组件基线.md)。14 个当前服务已统一接入 Actuator、Prometheus Registry、健康分组、核心业务/异步指标和具备集中安全边界的 `leetmodel.log.v1` JSON 日志，并完成受保护抓取、六类版本化看板、22 条主动告警、SkyWalking 中央日志接收、同步 HTTP/Feign/JDBC Trace、异步物理 attempt Trace 以及告警到业务事实的联合演练；中央 audit-service 的查询能力仍待实施。
