...
> 设计状态：目标设计已确认，运行时尚未实施 SkyWalking、Prometheus、Grafana、Alertmanager 和中央 audit-service。当前事实是项目已有业务 `traceId`、部分 Actuator/Micrometer 指标、AI 调用审计和可靠消息运维页面，但没有完整的采集、时序存储、Trace 拓扑、中央日志和主动告警闭环。
...
1. 统一 Actuator、Prometheus Registry、liveness/readiness/degraded 语义和服务资源字段。
2. 部署 Prometheus、Grafana、Alertmanager，建立系统总览和 MVP 主链看板。
3. 按 [日志系统](日志系统.md) 统一 JSON 日志，再接入 SkyWalking OAP、遥测存储和 UI。
4. 验证 Gateway、Spring MVC/WebFlux、Feign、JDBC、RocketMQ 的 Agent 兼容矩阵，再补齐自定义异步 Span。
