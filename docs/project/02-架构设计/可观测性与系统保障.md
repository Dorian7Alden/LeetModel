...
> 设计状态：目标设计已确认，已建立可重复的本地 SkyWalking/BanyanDB/Prometheus/Grafana/Alertmanager 环境，详见 [可观测性组件基线](可观测性组件基线.md)。13 个当前服务已统一接入 Actuator、Prometheus Registry、健康分组、核心业务/异步指标和 `leetmodel.log.v1` JSON 日志，并完成受保护抓取、六类版本化看板、22 条主动告警及 Runbook 闭环；日志安全与中央接收、完整 Trace 和中央 audit-service 仍待实施。
...
5. 以 [日志系统](日志系统.md) 已落地的统一 JSON 日志为基线，补齐集中脱敏/限频，再接入 SkyWalking OAP 日志接收与查询。
