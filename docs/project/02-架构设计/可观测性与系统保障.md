...
6. 13 个服务由同一开关附加 Agent；典型同步链已真实验证 Gateway → team-service → Feign → user/problem → MySQL，并可由业务 `traceId`、`swTraceId` 和中央日志双向检索。真实 Agent/RocketMQ 门禁也已证明 Outbox 成功/重试、Inbox consumed/duplicate、正常/接管租约和 AI UNKNOWN 形成独立有界 Trace，且不含业务主键 tag。
