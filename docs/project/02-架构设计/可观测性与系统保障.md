...
> 设计状态：目标设计已确认，已建立可重复的本地 SkyWalking/BanyanDB/Prometheus/Grafana/Alertmanager 环境，详见 [可观测性组件基线](可观测性组件基线.md)。13 个当前服务已统一接入 Actuator、Prometheus Registry、健康分组和核心业务/异步指标，并完成受保护抓取、六类版本化看板和 Alertmanager 基础路由；生产告警、中央日志和中央 audit-service 仍待实施。
...
| 遥测管道（当前）/审计与日志管道（后续） | Prometheus、Alertmanager、Grafana、OAP 自身健康与抓取质量；审计和日志落地后补充归档延迟、非法 schema、上报失败与丢弃 |
...
| 域 | 核心指标 | 允许标签 |
|----|----------|----------|
| 可靠消息 | `leetmodel.messaging.outbox/inbox/publish/consume/consumer/dlq.*` | 固定 Topic、当前服务消费组、状态、结果、`claim_type` |
...
本地 Prometheus 通过版本化 file-SD 固定发现 13 个服务，同时抓取自身、SkyWalking OAP、Alertmanager 和 Grafana。业务服务运行在宿主 JVM，Prometheus 与 Grafana 因而使用 host network 并显式只监听 `127.0.0.1`；OAP 遥测也只映射到 loopback。Prometheus 从 `/run/secrets/management_token` 文件读取 `X-LeetModel-Management-Token`，仓库不保存 Token；`start-observability.sh` 在忽略目录 `.observability-runtime/` 生成或复用 Token，`start-mvp.sh` 自动向 13 个服务注入同一值。静态目标不经 Gateway，Gateway 配置不得出现 Actuator 下游路由。

Grafana 通过只读 provisioning 加载系统总览、MVP 主链、AI 资源与稳定性、异步任务、可靠消息和遥测管道六类看板。Prometheus 当前只加载用于目标数和可用数的基础记录规则；Alertmanager 已按 `critical/warning` 分级路由，并在同一 `alertname + service` 上用 critical 抑制 warning。本地接收器为空，不代表生产通知已经配置。阈值、告警注解、真实通知目标和 Runbook 属于下一阶段，HTTP SLO 仍必须等待真实基线。
...
4. 部署 Prometheus、Grafana、Alertmanager，建立系统总览和 MVP 主链看板。
5. 按 [日志系统](日志系统.md) 统一 JSON 日志，再接入 SkyWalking OAP 日志接收与查询。
6. 按 [操作审计架构](操作审计架构.md) 建立审计契约、专用 Topic、audit-service 和管理查询。
7. 以用户角色变更验证“业务变更+审计 Outbox”同事务，以 Consumer 暂停和 DLQ 重放验证外部副作用两阶段审计。
8. 执行 OAP 中断、Prometheus 中断、审计消费中断、日志队列溢出、租约接管和 AI UNKNOWN 故障演练。
