...
> 设计状态：D-02 已确认职责边界。当前 new-api 已独立部署，LeetModel 调用链尚未切换，现有代码仍直连 DeepSeek 和 Kimi。S1 系列任务负责协议核验、适配、冒烟和旧直连收口。
...
当前 `DeepSeekAdapter` 和 `KimiAdapter` 仍直连官方接口，配置中仍有供应商 Base URL、API Key 和供应商路由。new-api 当前只是 Docker 基础设施。

迁移按 S1 系列任务执行：先完成初始化和黑盒验证，再增加安全配置和兼容适配器，随后建立逻辑模型绑定、错误与请求 ID 关联，完成客服文本和论文评审多模态冒烟，最后删除旧直连或提供显式、可审计的短期回退。迁移期间禁止同请求双调用和隐式回退，最终默认链路只有 new-api。
