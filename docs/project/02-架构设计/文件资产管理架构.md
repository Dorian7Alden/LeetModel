...
实线表示当前已经存在的入口关系，虚线表示业务文件迁移后的目标关系；管理端到 file-service 的代理链路与业务服务内部登记接口已落地，业务服务实际改用 fileId 仍待迁移。file-service 是文件资产控制面，不应成为所有二进制字节必经的网络中转站。
...
3. 已迁移管理员手动素材和题目附件，验证了上传、绑定、访问与删除闭环。
4. 再迁移头像和正式论文文件。
