...
> 设计状态：控制面基础能力已实现，题目附件、用户头像与正式论文已完成迁移。file-service 已独立拥有 `lm_file`，提供手动资产、历史盘点、临时访问、延迟清理、业务文件登记、交接目录接管与跨服务引用投影；problem-service、user-service 与 submission-service 均已改为只保存 fileId 并发布绑定/解绑事件。本文同时描述后续目标架构与迁移边界，业务服务仍不得让二进制流量全部经过 file-service。
