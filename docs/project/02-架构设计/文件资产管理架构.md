...
> 设计状态：第一阶段控制面已实现，业务文件迁移仍未开始。当前题目附件、头像、正式论文等业务关系仍由各自服务保存对象路径；file-service 已独立拥有 `lm_file`，提供手动资产、历史盘点、临时访问和延迟清理。本文同时描述后续目标架构与迁移边界，不能把未迁移的业务关系当作当前事实。
...
实线表示当前已经存在的入口关系，虚线表示业务文件迁移后的目标关系；管理端到 file-service 的第一阶段代理链路已落地。file-service 是文件资产控制面，不应成为所有二进制字节必经的网络中转站。
