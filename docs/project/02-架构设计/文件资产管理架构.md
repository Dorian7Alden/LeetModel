...
> 设计状态：控制面基础能力已实现，题目附件与用户头像已完成迁移；正式论文仍保存对象路径。file-service 已独立拥有 `lm_file`，提供手动资产、历史盘点、临时访问、延迟清理、业务文件登记与跨服务引用投影；problem-service 与 user-service 已改为只保存 fileId 并发布绑定/解绑事件。本文同时描述后续目标架构与迁移边界，不能把尚未迁移的业务关系当作当前事实。
