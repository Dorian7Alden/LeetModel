...
> 设计状态：控制面基础能力已实现，业务文件迁移仍未开始。当前题目附件、头像、正式论文等业务关系仍由各自服务保存对象路径；file-service 已独立拥有 `lm_file`，提供手动资产、历史盘点、临时访问、延迟清理、业务文件登记与跨服务引用投影。本文同时描述后续目标架构与迁移边界，不能把未迁移的业务关系当作当前事实。
