...
> 设计状态：控制面基础能力已实现，题目附件、用户头像与正式论文已完成迁移，预签名分片直传会话已落地。file-service 已独立拥有 `lm_file`，提供手动资产、历史盘点、临时访问、延迟清理、业务文件登记、交接目录接管、预签名直传会话与跨服务引用投影；problem-service、user-service 与 submission-service 均已改为只保存 fileId 并发布绑定/解绑事件。本文同时描述后续目标架构与迁移边界，业务服务仍不得让二进制流量全部经过 file-service。
...
flowchart LR
    ...
    adminService -. "管理命令与查询" .-> fileService
    domainServices -. "创建资产、登记绑定与申请访问授权" .-> fileService
    fileService --> assetDb
    fileService --> minio
    businessWeb -. "预签名直传和下载" .-> minio
    internalConsumer -. "受控临时访问" .-> minio
```

图中关系均已落地：管理端通过 admin-service 代理 file-service 执行上传、查询、访问和删除；题目附件、用户头像与正式论文由业务服务按用途登记、接管或绑定，只保存 fileId 并发布绑定/解绑事件；大文件由客户端按 file-service 签发的预签名地址直传对象存储。file-service 是文件资产控制面，不应成为所有二进制字节必经的网络中转站。
...
1. 建立 file-service、lm_file、文件资产 API 和管理端代理入口。已完成。
2. 扫描现有 Bucket，将历史对象登记为 `DISCOVERED`，此状态默认禁止删除；业务登记接口接管历史盘点资产时补齐归属并进入可用未绑定状态。已完成。
3. 迁移管理员手动素材和题目附件，验证上传、绑定、访问与删除闭环。已完成，题目附件库表只保留 file_id。
4. 迁移头像和正式论文文件。已完成，user-service 与 submission-service 分别只保存 `avatar_file_id` 与 `file_id`。
5. 论文临时分片继续由 submission-service 管理。仍保持该过渡边界，合并后的正式论文由 file-service 接管。
6. 各业务完成迁移后回收其正式资产 Bucket 的宽权限凭证。problem-service、user-service 与 AI 服务已移除对象存储配置；submission-service 仅保留临时分片所需凭证。
7. 对账任务持续比较 MinIO 对象、file_asset 和业务绑定事件，显式报告缺失对象与未知对象。已实现并接入延迟清理调度。

迁移完成后，业务服务不得再保存 `object_key`、`object_name` 或 `avatar_path` 等对象路由，正式文件统一以稳定 fileId 引用；新增文件用途必须先进入 `FilePurpose` 映射，由 file-service 决定命名空间、访问级别和允许的媒体类型。论文临时分片是当前唯一保留的对象路径例外。
