# paper-lifecycle-design

## paper-lifecycle-design

> 状态：设计中。论文生命周期状态机的基础设计，评审执行编排、改进建议、重新评审另起任务推进。
>
> 对应问题域：`docs/project/analysis/paper-review-problem-space.md` 域 4 的 4-1 状态机设计、跨切面 C-5 状态单一事实来源。同时统一域 1 与域 2 已定稿文档中散落的状态表述，不改变已定稿的语义。
...
- 改进建议是评审完成后的 VIP 增值动作，由 suggestion 服务独立承载，不进入本状态机，见决策记录 6。
- 评审内部的工作流编排，检索增强、分段评审、汇总打分等步骤，属域 4-2，本状态机只到评审中这一粒度，见决策记录 8。
...
| 评审开始 | 待评审 | 评审中 | review 服务 | 无 |
| 评审完成 | 评审中 | 已完成 | review 服务 | 评审结果入库 |
| 评审失败 | 评审中 | 失败 | review 服务 | 记录失败阶段为评审 |
| 超时回收 | 全部四个中间态 | 失败 | 看门狗定时任务 | 记录失败阶段与超时原因 |

规则要点：

- 解析部分降级放行评审，缺页信息随解析产物的质量通道传递，与解析设计定稿一致。
...
看门狗兜底：定时任务扫描停留在任一中间态超过阈值的记录，置为失败并释放互斥，阈值按状态分别参数化。待解析与待评审的超时覆盖触发解析失败、评审任务入队丢失的场景，解析中与评审中的超时覆盖消费中宕机、回调丢失的场景，防止队伍被永久繁忙锁死。评审链路的洪峰排队修正：待评审与评审中的合法排队等待可能远超固定阈值（洪峰容量模型见评审工作流文档第七节），这两个状态的超时判定在时间超阈值之外，追加一次 review 侧任务存在性查询（`GET /internal/review/tasks/{submissionId}/exists`），任务存在即视为合法排队不回收，精确区分入队丢失与洪峰排队，避免批量误杀。扫描的比较基准是进入当前状态的时间，落 stage_time 字段，每次流转随状态同一条更新语句写入。完整的消费恢复机制属域 3-6，本状态机只保证卡死可自愈，见决策记录 7。

---

### 六、API 契约

状态字段在 submission 服务，事件接口全部挂载 submission 服务。解析执行方尚未决策宿主，其调用方身份不影响契约。
...
- 错误码新增 40608 非法状态流转，续用 BB=06 提交号段；评审侧业务错误码留域 4 后续设计启用 07 号段。
...
**原因**：改进建议是 VIP 功能、可多次发起、可在评审完成后任意时刻发起，与一次性单向推进的评审生命周期形态完全不同；纳入状态机会把终态变成中间态，破坏互斥释放语义。suggestion 服务后续以 submissionId 关联已完成的提交独立建模。
...
- 评审任务表结构、MQ 消息结构、生产与消费可靠性，属域 3。
- 评审工作流编排与断点恢复，属域 4-2。
- 评审失败后的自动重试与用户手动重新评审入口，属域 4-9，本期失败即终态、重新提交即新生命周期。
- 重新解析与状态机的衔接，PARTIAL 重试、解析器升级产出新 parseId 后是否更新提交记录，属域 2-8。
- 改进建议阶段的建模，suggestion 服务设计时推进。
- 评审进度的前端呈现方式，轮询或 SSE，属域 6-2。
- 看门狗超时阈值数值、事件接口的认证方式，实现时确定。
...
- 上传协议、队伍互斥与 submission 表结构：`docs/project/modules/submission-service-design.md`
- 解析三态与状态衔接语义：`docs/project/modules/pdf-parsing-design.md`
