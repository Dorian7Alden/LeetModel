...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **JDK** | Java 17 LTS | 17.0.x | 企业主流 LTS，Spring Boot 3.x 基线要求，面试高频（Record、Sealed Class、Pattern Matching、Text Block） | ✅ 已集成 |
| **Spring Boot** | Spring Boot | 3.3.x | 3.x 要求 Java 17+，2.7.x 已停止维护 | ✅ 已集成 |
| **Spring Cloud** | 微服务框架规范 | 2023.0.x | 提供 Gateway、LoadBalancer 等微服务基础设施抽象 | ✅ 已集成 |
| **Spring Cloud Alibaba** | 阿里中间件集成 | 2023.0.1.x | 实现 Spring Cloud 规范，集成 Nacos（注册/配置中心） | ✅ 已集成 |
...
| 组件 | 选型 | 选型理由 | 状态 |
|------|------|---------|------|
| **构建工具** | Maven | 国内 Java 岗位 90%+ 用 Maven，多模块项目通过父 POM 统一依赖版本管理，面试展示标准的工程化能力 | ✅ 已集成 |
| **项目结构** | Maven 多模块 | 1 个父 POM + 3 个公共 jar 模块 + 1 个网关 + 9 个业务微服务模块 | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **数据库** | MySQL | 8.0+ | 国内 Java 岗位事实标准，面试必考索引、锁机制、事务隔离级别、SQL 优化 | ✅ 已集成 |
| **ORM** | MyBatis-Plus | 3.5.7+ | 零 SQL 写 CRUD + 复杂查询手写 SQL 灵活可控，比 JPA 更贴合国内实际开发习惯 | ✅ 已集成 |
| **连接池** | HikariCP | 随 Spring Boot | Spring Boot 3.x 默认，Java 生态最快连接池，无需额外配置 | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **缓存中间件** | Redis | 7.x | Java 生态唯一选择，用于 Token 黑名单、验证码、热点数据缓存、排行榜缓存 | ✅ 已集成 |
| **客户端** | Lettuce | 随 Spring Boot | Spring Boot 3.x 默认，基于 Netty 实现响应式/非阻塞，连接池天然优于 Jedis | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **文件存储** | MinIO | RELEASE.2024-08-17 | 开源 S3 兼容对象存储，Docker 一键部署，本地开发零成本。代码写标准 S3 SDK，将来迁移阿里云 OSS/腾讯云 COS 零代码改动。版本固定（不用 latest），保证环境可复现 | 📋 版本锁定 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **消息队列** | Apache RocketMQ | 5.x | 阿里开源，Java 生态原生，事务消息、延迟消息开箱即用。本项目的评审异步回调、排名更新一致性、操作日志采集均依赖 MQ | 📋 版本锁定 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **注册中心** | Nacos | 2.x | 注册中心 + 配置中心一体，支持 AP/CP 模式切换 | ✅ 已集成 |
| **配置中心** | Nacos | 2.x | 配置热更新、多环境配置隔离（dev/test/prod） | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **网关** | Spring Cloud Gateway | 随 Spring Cloud | 基于 WebFlux 响应式，性能优于 Zuul（已停更） | 📋 版本锁定 |

### 5.3 认证鉴权

| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **认证框架** | Sa-Token + Spring Security 6 | sa-token 1.37+ | Sa-Token 轻量权限框架，注解式鉴权（`@SaCheckRole("VIP")`），天然支撑 VIP/普通用户/管理员三级 RBAC | ✅ 已集成 |
| **令牌** | JWT + Redis 黑名单 | - | JWT 无状态签发 + Redis 黑名单实现主动失效（踢人下线、强制登出） | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **流量控制** | Sentinel | 1.8.x | Spring Cloud Alibaba 生态原生集成，滑动窗口限流 + 熔断降级 + 控制台实时监控 | 🔮 预研 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **LLM 框架** | LangChain4j | 0.34+ | Java 版 LangChain，功能完整（对话链、RAG、工具调用、Embedding），社区活跃度高于 Spring AI | 📋 版本锁定 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **搜索引擎** | Elasticsearch | 8.x | 全文检索 + 向量检索一体，一个引擎同时支撑题目搜索和 RAG 知识库检索 | 🔮 预研 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **API 文档** | Knife4j | 4.5.0 | 基于 SpringDoc OpenAPI 2.x 增强，现代 UI + 在线调试，后端独立开发时一键接口测试 | ✅ 已集成 |
| **外部调试** | Apifox | - | 作为辅助工具，与 Knife4j 互补 | 🔮 预研 |
...
| 组件 | 版本 | 选型理由 | 状态 |
|------|------|---------|------|
| **Lombok** | 1.18.x | Spring Boot 标配，编译期生成 getter/setter/Builder/日志，避免样板代码 | ✅ 已集成 |
| **MapStruct** | 1.5.5+ | 编译期生成对象映射代码，零反射，比 BeanUtils 快 10-20 倍，字段名对不上编译即报错 | ✅ 已集成 |
| **Hutool** | 5.8.x | Java 通用工具库，字符串/日期/JSON/文件/加密一把梭，国内项目事实标准 | ✅ 已集成 |
...
| 组件 | 选型 | 版本 | 选型理由 | 状态 |
|------|------|------|---------|------|
| **Metrics** | Micrometer | 随 Spring Boot 3.x | Spring Boot 3.x Actuator 默认 metrics 门面 | ✅ 已集成 |
| **链路追踪** | Micrometer Tracing | 随 Spring Boot 3.x | 自动注入 traceId/spanId 到日志（MDC），替代已停更的 Sleuth | ✅ 已集成 |
| **日志框架** | SLF4j + Logback | 随 Spring Boot | Lombok `@Slf4j` + Logback 配置，通过 `logging.pattern.level` 在每条日志中输出 traceId | ✅ 已集成 |
...
| 组件 | 选型理由 | 状态 |
|------|---------|------|
| **JUnit 5** | Spring Boot 3.x 默认测试框架 | ✅ 已集成 |
| **Mockito** | Mock 外部服务调用，单元测试核心 | ✅ 已集成 |
| **Testcontainers** | Docker 启动真实 MySQL/Redis/RocketMQ 做集成测试，测试环境 = 生产环境 | ✅ 已集成 |
| **Spring Boot Test** | 切片测试（`@WebMvcTest`、`@DataJpaTest`），按层隔离测试 | ✅ 已集成 |
...
| 组件 | 用途 | 状态 |
|------|------|------|
| **Docker Compose** | 本地开发环境编排，一键启动 MySQL + Redis + RocketMQ + Nacos + ES + MinIO | 🔮 预研 |
| **Dockerfile**（每服务一个） | 多阶段构建：Stage 1 Maven 编译 → Stage 2 JDK 17 slim 镜像运行 | 🔮 预研 |
| **K8s（预留）** | 面试提一句"后续可迁移到 K8s 做服务编排"，实际不做 | 🔮 预研 |
...
| 框架 | 版本 | 兼容要点 | 当前状态 |
|------|------|---------|---------|
| JDK | 17 LTS | Spring Boot 3.x 硬性要求 Java 17+ | ✅ 已集成 |
| Spring Boot | 3.3.x | Jakarta EE 迁移（`javax.*` → `jakarta.*`） | ✅ 已集成 |
| Spring Cloud | 2023.0.x | 对应 Spring Boot 3.3.x | ✅ 已集成 |
| Spring Cloud Alibaba | 2023.0.1.x | 对应 Spring Cloud 2023.0.x，Nacos 2.x，Sentinel 1.8.x | ✅ 已集成 |
| MyBatis-Plus | 3.5.7+ | 必须用 `-spring-boot3-starter` artifact 变体 | ✅ 已集成 |
| MyBatis-Plus JSqlParser | 3.5.7+ | 分页插件依赖，不可遗漏 | ✅ 已集成 |
| Sa-Token | 1.38.0+ | 必须用 `-spring-boot3-starter` 变体 | ✅ 已集成 |
| Micrometer Tracing | 随 Boot 3.x | Spring Boot 3.x 内置，替代 Sleuth | ✅ 已集成 |
| Knife4j | 4.5.0 | artifact 必须带 `jakarta` 后缀 | ✅ 已集成 |
| MapStruct | 1.5.5+ | 1.5.x 以下不支持 Java 17 Record | ✅ 已集成 |
| LangChain4j | 0.34+ | 与 Spring Boot 3.x 无版本冲突 | 📋 版本锁定 |
| Elasticsearch | 8.x | 与 `spring-boot-starter-data-elasticsearch` 集成 | 🔮 预研 |
| RocketMQ | 5.x | RocketMQ Spring Boot Starter 已适配 Boot 3.x | 📋 版本锁定 |
...
| 组件 | 状态 | 说明 |
|------|------|------|
| Spring Boot 3.3.5 | ✅ 已集成 | 根 POM parent |
| Spring Cloud 2023.0.3 | ✅ 已集成 | 根 POM dependencyManagement BOM |
| Spring Cloud Alibaba 2023.0.1.0 | ✅ 已集成 | 根 POM dependencyManagement BOM |
| Nacos Discovery | ✅ 已集成 | user / problem 模块依赖 |
| MyBatis-Plus 3.5.7 | ✅ 已集成 | user 模块依赖 |
| MySQL Connector | ✅ 已集成 | user 模块依赖 |
| Flyway | ✅ 已集成 | user 模块依赖（V1 + V2 迁移） |
| Sa-Token 1.38.0 | ✅ 已集成 | common-security 模块依赖 |
| Spring Security 6 | ✅ 已集成 | common-security 模块依赖 |
| Knife4j 4.5.0 | ✅ 已集成 | user 模块依赖 |
| Lombok | ✅ 已集成 | 根 POM 全局依赖 |
| MapStruct 1.5.5 | ✅ 已集成 | 根 POM 注解处理器 |
| Hutool 5.8.29 | ✅ 已集成 | common-core 依赖 |
| Testcontainers | ✅ 已集成 | user 模块测试依赖 |
| Micrometer Tracing | ✅ 已集成 | Spring Boot 3.x 内置 |
| Spring Cloud Gateway | 📋 版本锁定 | 根 POM BOM 间接引入，无模块依赖 |
| Spring Cloud LoadBalancer | ✅ 已集成 | user 模块依赖 |
| OpenFeign | ✅ 已集成 | common-api 模块依赖 |
| RocketMQ | 📋 版本锁定 | 根 POM 声明版本，无模块实际依赖 |
| LangChain4j | 📋 版本锁定 | 根 POM 声明版本，无模块实际依赖 |
| MinIO | 📋 版本锁定 | 根 POM 声明版本，无模块实际依赖 |
| Elasticsearch | 🔮 预研 | 文档提及，尚未引入 POM |
| Sentinel | 🔮 预研 | 文档提及，尚未引入 POM |
| Redis | 📋 版本锁定 | common-security 声明依赖但未配置连接 |
| Docker Compose | 🔮 预研 | 尚未创建 compose 文件 |
