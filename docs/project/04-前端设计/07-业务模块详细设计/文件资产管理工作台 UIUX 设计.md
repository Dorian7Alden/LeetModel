...
>
> 适用入口：管理端内容中心的“文件”完整工作面。当前入口为 `/admin/content?view=files`，前端通过 `admin-service` 访问 file-service；本文只定义管理端体验，不改变文件资产的数据所有权和 MinIO 存储边界。
...
文件资产完整工作面采用“左侧分组 + 右侧资产列表 + 预览抽屉”的结构：

```mermaid
flowchart LR
    Entry[内容中心 / 文件工作面]
    Entry --> Header[资产事实、历史盘点、刷新、上传文件]
    Header --> Group[分组导航]
...
```text
┌──────────────────────────────────────────────────────────────┐
│ 33 个文件 · 182 MB · 手动 20 · 只读 13 [盘点历史] [上传文件] │
├───────────────┬──────────────────────────────────────────────┤
...
资产数量、容量、手动上传数和只读数使用单行事实区展示，不使用统计卡。管理员最常用的查找、预览和处理操作必须优先可见。
...
| 列 | 内容 |
|---|---|
| 文件 | 文件图标、原始文件名、扩展名、文件大小和可复制文件 ID |
| 分组 | `icons`、`campaign/2026` 或未分组 |
| 来源 / 状态 | 手动上传或历史盘点；可用、只读、等待清理或清理失败 |
| 更新时间 | 登记或状态变更时间 |
| 操作 | 预览和更多菜单；下载、复制与删除收入更多菜单 |
...
点击“预览”打开右侧抽屉，不离开当前列表。抽屉展示文件名、文件 ID、类型和大小，操作区提供复制 ID、复制临时链接和下载；分组、Bucket、Object Key 与生命周期收入默认折叠的技术详情。删除只保留在列表更多菜单中，避免预览任务夹带高风险动作。
...
关闭上传抽屉不得清空队列。存在待处理或失败项时，主工作面持续显示紧凑队列状态和“查看队列”入口；只有管理员主动移除文件或上传成功后，队列项才可消失。
