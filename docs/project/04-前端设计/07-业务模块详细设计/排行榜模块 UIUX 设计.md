...
5. **按需懒加载与数据加载控制规范**：
   - **视图级生命周期解耦**：总览模式仅加载全局指标与天梯概览；单题细览仅拉取目标赛题的排榜快照，杜绝初入页面时一次性预载全量赛题与无关榜单。
   - **渐进式异步渲染 (Progressive Loading)**：细览页首屏优先秒级呈现核心排名表与队伍名次；分数分布直方图与环形图采用异步渐进拉取，图表区展示轻量局部骨架，绝不阻塞用户查榜。
   - **服务端分页与防抖懒加载 (Server-side Lazy Query)**：赛题可视化筛选中心不再在客户端持有全量题目，只有打开弹窗时才向后端按需发起带防抖 (300ms) 的分页查询（`getPublicProblemList({ page, pageSize: 6, keyword, contestId, year, difficulty })`），彻底解决海量题目性能瓶颈。
   - **题面速览抽屉前端内存缓存 (In-Memory LRU Cache)**：用户在看榜过程中临时速览题面时，已加载的题目详情自动缓存至内存字典，二次点击直接秒开，零网络延迟，零重复请求。
6. **全链路数据分页 (Pagination Coverage)**：
   - **细览榜单分页**：细览排名表格默认每页 10/20/50 条，并实现“定位我的队伍”自动计算目标页码并跨页平滑定位高亮；
   - **总览天梯分页**：全平台赛题天梯榜支持多维排序与标准分页器，从容承载海量赛题。
7. **多样化可视化图表**：纯 CSS/SVG 极速渲染，包含分数分布梯队直方图、竞赛实力层级 Donut 环形构成图、赛题参赛规模横向对比柱状图与均分峰值区间图。
...
- [x] 扩展前端 API 层支持分数分布查询与类型定义
- [x] 编码实现 `RankingPage.vue` 重构（头部、工具栏、指标图表、领奖台、数据表、骨架屏、状态异常）
- [x] 本地构建与静态检查验证
- [x] 验收与文档索引归档
