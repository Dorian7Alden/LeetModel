...
- 当前实现：左侧以“正在组建、练习中、已结束”作为三个独立下拉组并展示各组全部队伍，三个状态分别拥有独立子页面路由；中间只展示当前选中队伍，组建态已经集成原私有详情页的资料编辑、成员职责、提交权限、招募、申请审核和开赛管理，练习中和已结束使用“题目、提交、排行”页内导航切换单一内容区；右侧持续展示队伍简介统计，队伍广场保持独立入口。
...
flowchart LR
    ...
    roles --> start[队长开始练习]
    start --> active[练习中]
    active --> upload[上传并管理论文版本]
    upload --> aiReview[发起并等待 AI 评审]
    aiReview --> improve[查看问题与改进建议]
    improve --> upload
    active --> finish[到期或队长提前结束]
    finish --> ended[已结束]
...
| `IN_PROGRESS` | 练习中 | 在截止前完成有效论文版本 | 倒计时、提交权限、最新版本、评审进度 | 上传论文或查看评审 |
...
- 顶部导航固定提供“题目、提交、排行”三个选项，当前项使用文字、图标和底部指示线共同表达。
- 题目内容：显示业务题号、标题、难度、时长、原始 Markdown 题面和附件，并提供打开完整题目的入口。
- 提交内容：提交权限可用时直接选择 20MB 以内 PDF、查看上传进度和提交下一版本；下方持续展示最新版本、评审状态和版本记录。
...
已结束子页面保持与练习中一致的题目、提交、排行导航关系，降低生命周期切换成本，但内容目标改为复盘：题目内容回看原始要求，提交内容突出最终版本、评分状态和全部历史，排行内容突出当前队伍名次与同题参照。没有最终版本、成功评审或有效排名时使用明确空状态，不绘制伪趋势或伪百分位。
