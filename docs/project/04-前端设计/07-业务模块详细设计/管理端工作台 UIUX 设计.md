...
实施状态（2026-09-14）：`/admin/content` 已以 `view=problems|contests|tags|files|knowledge` 承载五个可恢复工作面，旧的 `view=storage` 会归一为 `view=files`。页面移除 Hero、指标卡和说明型占位内容，题目列表已支持关键字、赛事、年份、语言、难度与状态组合筛选；题目、赛事和标签的新增或编辑统一使用右侧面板。文件上传队列关闭面板后仍保留并可重新进入；知识库目录、文档与真实索引事实同屏展示，目录和索引请求分别处理失败，任何请求失败均不回退到伪造的别名、版本、数量或健康状态。
