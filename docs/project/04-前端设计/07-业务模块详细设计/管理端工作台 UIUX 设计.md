...
实施状态（2026-09-14）：`/admin/operations` 已以 `view=submissions|teams|reviews|suggestions|rankings` 承载五个可恢复工作面，默认提交视图不写冗余查询参数。提交、队伍、评审和建议统一标明“最近 50 条”并提供样本内组合筛选；提交预览按需取得临时地址，必要内部标识只在详情面板展示和复制。管理聚合层已提供按关联 ID 批量查询队伍摘要的接口，避免最近样本超出“最近队伍”集合后业务名称全部缺失；队伍成员数使用真实统计，缺失时显示 `—`。建议与评审按异常状态优先展示，失败依据进入详情。排行保留紧凑全局事实表，进入单题后展示分数分布、成绩分段和队伍明细，重建前确认题目与读取影响；历史排行缺少业务题目资料时明确显示“题目资料未取得”，不把内部 ID 包装成业务标题。
...
实施状态（2026-09-14）：`/admin/ai` 已以 `view=calls|versions|evaluations|production|messaging` 承载五个可恢复工作面，默认调用视图不写冗余查询参数。页首只显示成功率、失败调用、平均耗时、Token 和当前生产版本，调用统计与生产快照独立降级。调用观测使用服务端分页、筛选和同口径聚合，业务功能、操作、模型、状态与时间作为默认扫描轴，完整 Call ID、Trace ID、版本快照、Token 拆分、耗时、费用和失败依据进入详情面板；图表只保留模型调用分布与平均耗时。版本目录以功能注册表呈现 owner 服务真实发布状态，未知能力不生成版本，输入输出契约按需展开。质量评价以测试集、评价任务和版本对比分步组织，内部标识进入详情，重试、暂停、恢复、取消和评分重算均在对象详情内完成。生产版本把当前指针压缩为单行事实，候选、历史和审计互斥切换，继续执行服务端预检、差异确认、revision 校验和应用协议。消息运维先选择告警服务，再在消费器、Outbox、Inbox、死信和 Trace 之间单面切换；暂停需确认，重放需填写原因，并为加载和操作失败保留原位反馈。所有工作面在 1024px PC 桌面宽度下不产生页面级横向溢出。
...
实施状态（2026-09-14）：`/admin/audit` 已从时间、服务、操作、风险和结果筛选直接进入事件表，操作者、目标与链路标识保留为渐进展开的精确条件。审计查询当前是有界最近 50 条而非完整分页；页面在 `hasMore` 时明确提示仍有更早记录并引导缩小筛选范围，不把样本包装为全量。结果表以时间、操作、结果、目标和操作者为扫描轴，来源服务与阶段作为次级事实，审计事件 ID 不占默认列。点击事件后以 operationId 补查同一操作的可见阶段并按时间线切换，原因、失败码、白名单摘要字段差异和 Trace 标识在详情中核对。查询失败保留最近结果并显示中央审计故障态，真正空数据与筛选无结果使用不同状态；页面已通过 1024px PC 桌面无横向溢出检查。
