...
> 最后更新：2026-07-25
> 状态：已修订
> 功能需求见：[01-requirements.md](./01-requirements.md)
> 用例分析见：[03-usecase-analysis-results.md](./03-usecase-analysis-results.md)
...
│   ├── LeetModel-common/        ← 公共模块（jar 依赖，非独立服务）
│   │   ├── common-core/         ← 统一响应体、异常、工具类、分页
│   │   ├── common-security/     ← 认证鉴权
│   │   └── common-ai/           ← LLM调用 · RAG检索 · Prompt模板
│   ├── LeetModel-gateway/       ← API 网关（唯一对外入口）
│   ├── LeetModel-user/          ← 用户服务（注册登录、RBAC）
│   ├── LeetModel-team/          ← 团队服务（组队、解散）
│   ├── LeetModel-problem/       ← 题目服务（CRUD、全文搜索）
│   ├── LeetModel-assistant/     ← AI 智能客服（对话交互，功能可拓展）
│   ├── LeetModel-submission/    ← 作品提交与管理服务
│   ├── LeetModel-review/        ← 评审打分服务（多维度评分）
│   ├── LeetModel-suggestion/    ← 改进建议服务（可选VIP）
│   ├── LeetModel-ranking/       ← 排行榜服务（每题独立排名）
...
| **submission** | 8084 | 作品提交（上传+存储）、作品管理（历史+结果查询） | 查提交记录、关联题目信息 | 生产：评审任务 |
| **assistant** | 8085 | 对话式选题推荐、会话管理 | - | - |
| **review** | 8086 | 消费评审任务、多维度评分、结果回写 | 回写评审结果 | 消费：评审任务 · 生产：排名更新事件 |
| **suggestion** | 8087 | 改进建议生成（VIP 角色控制） | 接收建议请求 · 回写建议结果 | 生产 + 消费：建议任务 |
| **ranking** | 8088 | 每题独立排名、多维度分数展示 | 查排名 TopN、某用户排名 | 消费：排名更新事件 |
| **admin** | 8089 | 聚合统计、操作日志 | 无（纯聚合查询方） | 消费：各类业务事件 |
