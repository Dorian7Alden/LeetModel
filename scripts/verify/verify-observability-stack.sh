...
OBSERVABILITY_DIR="${OBSERVABILITY_CONFIG_DIR}"
RUNTIME_DIR="${OBSERVABILITY_RUNTIME_DIR}"
...
cleanup() {
  if [[ "${PROMETHEUS_STOPPED}" == "true" ]]; then
    docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" up -d --wait prometheus >/dev/null 2>&1 || true
  fi
...
docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" config --quiet
echo "[通过] Compose 配置"
...
expected_services='["admin-service","ai-assistant-service","ai-evaluation-service","ai-gateway-service","ai-review-service","ai-suggestion-service","audit-service","file-service","gateway-service","knowledge-retrieval-service","problem-service","ranking-service","submission-service","team-service","user-service"]'
if ! jq -e --argjson expected "${expected_services}" \
    'length == 15 and ([.[].labels.service] | sort | unique) == $expected' \
    "${targets_file}" >/dev/null; then
  echo "Prometheus 服务发现文件没有且仅有 15 个固定服务。" >&2
  exit 1
fi
echo "[通过] 15 个服务抓取目标"
...
if git -C "${REPO_ROOT}" ls-files --error-unmatch .runtime/backend/observability/management-token \
    >/dev/null 2>&1; then
...
"${INFRA_SCRIPTS_DIR}/start-observability.sh" >/dev/null
...
if [[ "${service_targets_ready}" != "true" ]]; then
  echo "Prometheus 未发现 15 个固定服务，或未抓取临时服务。" >&2
  exit 1
fi
...
echo "[通过] 15 服务发现、受保护抓取与 PromQL 查询"
...
docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" stop prometheus >/dev/null
...
docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" up -d --wait prometheus >/dev/null
PROMETHEUS_STOPPED=false
