...
for required in "${reporter}" "${metrics}" "${logback}" "${logging_config}" "${lal}" \
    "${oap_log4j}" "${mtail_program}" "${OBSERVABILITY_COMPOSE_FILE}"; do
  if [[ ! -s "${required}" ]]; then
...
if ! rg -q 'SW_NAMESPACE: leetmodel-observability' "${OBSERVABILITY_COMPOSE_FILE}" \
    || ! rg -q 'SW_SEARCHABLE_LOGS_TAG_KEYS:.*business_trace_id.*sw_trace_id.*domain_task_id.*ai_call_id' \
      "${OBSERVABILITY_COMPOSE_FILE}" \
    || ! rg -q 'skywalking-oap-logs:/skywalking/logs' "${OBSERVABILITY_COMPOSE_FILE}" \
    || ! rg -q 'skywalking-log-metrics:' "${OBSERVABILITY_COMPOSE_FILE}"; then
...
RUNTIME_DIR="${CONTRACT_RUNTIME_DIR}/skywalking-logging/${RUN_ID}"
SERVICE_PORT="${SKYWALKING_LOG_SERVICE_PORT:-18096}"
FAKE_OAP_PORT="${SKYWALKING_LOG_FAKE_OAP_PORT:-18098}"
TOKEN_FILE="${LEETMODEL_MANAGEMENT_TOKEN_FILE:-${OBSERVABILITY_RUNTIME_DIR}/management-token}"
