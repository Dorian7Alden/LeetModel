...
start_script="${DEV_SCRIPTS_DIR}/start-mvp.sh"
prepare_script="${INFRA_SCRIPTS_DIR}/prepare-skywalking-agent.sh"
...
RUNTIME_DIR="${CONTRACT_RUNTIME_DIR}/skywalking-tracing/${RUN_ID}"
...
cleanup() {
  ...
  if [[ "${OAP_STOPPED}" == "true" ]]; then
    docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" up -d --wait skywalking-oap >/dev/null 2>&1 || true
  fi
...
"${INFRA_SCRIPTS_DIR}/init-rocketmq.sh" >/dev/null
...
if [[ "${VERIFY_OAP_OUTAGE:-false}" == "true" ]]; then
  docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" stop skywalking-oap >/dev/null
  OAP_STOPPED=true
  ...
  docker compose -f "${OBSERVABILITY_COMPOSE_FILE}" up -d --wait skywalking-oap >/dev/null
  OAP_STOPPED=false
