...
RUNTIME_DIR="${SERVICE_RUNTIME_DIR}"
SKIP_BUILD=false
...
OBSERVABILITY_TOKEN_FILE="${LEETMODEL_MANAGEMENT_TOKEN_FILE:-${OBSERVABILITY_RUNTIME_DIR}/management-token}"
...
if [[ "${SKYWALKING_ENABLED}" == "true" ]]; then
  ...
  SKYWALKING_AGENT_JAR="$("${INFRA_SCRIPTS_DIR}/prepare-skywalking-agent.sh")"
...
"${INFRA_SCRIPTS_DIR}/init-rocketmq.sh"
...
AUDIT_DB_APP_PASSWORD="${AUDIT_DB_APP_PASSWORD_VALUE}" \
  "${INFRA_SCRIPTS_DIR}/init-audit-database.sh" bootstrap
...
for index in "${!services[@]}"; do
  ...
  if [[ "${service}" == "audit-service" ]]; then
    ...
    AUDIT_DB_APP_USERNAME="${AUDIT_DB_APP_USERNAME_VALUE}" \
    AUDIT_DB_APP_PASSWORD="${AUDIT_DB_APP_PASSWORD_VALUE}" \
      "${INFRA_SCRIPTS_DIR}/init-audit-database.sh" finalize
  fi
