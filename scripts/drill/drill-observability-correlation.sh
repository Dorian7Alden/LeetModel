...
RUN_DIR="${DRILL_RUNTIME_DIR}/correlation-drill/${RUN_ID}"
WEBHOOK_PORT="${OBSERVABILITY_CORRELATION_WEBHOOK_PORT:-19094}"
WEBHOOK_URL="http://127.0.0.1:${WEBHOOK_PORT}"
TARGET_FILE="${OBSERVABILITY_RUNTIME_DIR}/prometheus-targets/correlation-drill.json"
RULE_FILE="${OBSERVABILITY_RUNTIME_DIR}/prometheus-rules/correlation-drill.yml"
WEBHOOK_PID=""
TEST_PID=""
...
"${VERIFY_SCRIPTS_DIR}/verify-alerting-contract.sh" >/dev/null
...
agent_jar="$(${INFRA_SCRIPTS_DIR}/prepare-skywalking-agent.sh)"
...
for _ in {1..90}; do
  "${QUERY_SCRIPTS_DIR}/query-observability-correlation.sh" --alert-file "${OUTBOX_ALERT}" --fact-file "${OUTBOX_FACT}" --allow-gaps >"${RUN_DIR}/outbox-result.json"
  "${QUERY_SCRIPTS_DIR}/query-observability-correlation.sh" --alert-file "${AI_ALERT}" --fact-file "${AI_FACT}" --allow-gaps >"${RUN_DIR}/ai-result.json"
