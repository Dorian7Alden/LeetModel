  ...
  <div class="permission-list-page">
    <el-card shadow="never">
      <div class="action-bar">
        <el-input v-model="keyword" placeholder="按权限名搜索" clearable style="width: 260px" />
        <div class="actions-right">
          <el-button @click="fetchPermissions" :loading="loading">刷新</el-button>
          <el-button type="primary" @click="openCreateDialog"><el-icon><Plus /></el-icon>新增权限</el-button>
        </div>
      </div>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增权限' : '编辑权限'" width="480px" destroy-on-close>
      <el-form ref="formRef" :model="formModel" :rules="formRules" label-width="80px">
        <el-form-item label="权限名称" prop="name"><el-input v-model="formModel.name" maxlength="100" show-word-limit placeholder="请输入权限名称" /></el-form-item>
        <el-form-item label="编码" prop="code"><el-input v-model="formModel.code" maxlength="100" show-word-limit placeholder="请输入权限编码" /></el-form-item>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (dialogMode.value === "create") await createPermission(payload);
    else await updatePermission(formModel.id, payload);
    ElMessage.success(dialogMode.value === "create" ? "权限创建成功" : "权限更新成功");
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`确定删除权限"${row.name}"吗？`, "删除确认", { type: "warning" });
