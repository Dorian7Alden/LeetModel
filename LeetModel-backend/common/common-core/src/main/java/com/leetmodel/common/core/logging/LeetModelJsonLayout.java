 ...
 * Throwable message；摘要脱敏与限频由后续日志安全层统一处理。</p>
 */
public final class LeetModelJsonLayout extends LayoutBase<ILoggingEvent> {
    ...
    public String doLayout(ILoggingEvent event) {
        ...
        value.put("message", event.getFormattedMessage());
        value.put("service", service);
        value.put("environment", environment);
        value.put("serviceVersion", serviceVersion);
        value.put("instance", instance);
        value.put("logger", event.getLoggerName());
        value.put("thread", event.getThreadName());

        value.put(LogFieldNames.TRACE_ID, text(structured, mdc, LogFieldNames.TRACE_ID));
        value.put(LogFieldNames.SW_TRACE_ID, text(structured, mdc, LogFieldNames.SW_TRACE_ID));
        value.put(LogFieldNames.SW_SPAN_ID, text(structured, mdc, LogFieldNames.SW_SPAN_ID));
        value.put(LogFieldNames.REQUEST_ID, text(structured, mdc, LogFieldNames.REQUEST_ID));
        value.put(LogFieldNames.OPERATION_ID, text(structured, mdc, LogFieldNames.OPERATION_ID));
        ...
        value.put(LogFieldNames.BUSINESS_ID, text(structured, mdc, LogFieldNames.BUSINESS_ID));
        value.put(LogFieldNames.DOMAIN_TASK_ID, text(structured, mdc, LogFieldNames.DOMAIN_TASK_ID));
        value.put(LogFieldNames.ATTEMPT_NO, integer(structured, mdc, LogFieldNames.ATTEMPT_NO));
        value.put(LogFieldNames.EVENT_ID, text(structured, mdc, LogFieldNames.EVENT_ID));
        value.put(LogFieldNames.AI_CALL_ID, text(structured, mdc, LogFieldNames.AI_CALL_ID));
    ...
    private String text(Map<String, Object> structured, Map<String, String> mdc, String key) {
        ...
        if (structuredValue != null) return String.valueOf(structuredValue);
    ...
    private List<String> stackFrames(IThrowableProxy throwable) {
        ...
        for (int index = 0; index < size; index++) {
            safeFrames.add(frames[index].getStackTraceElement().toString());
        }
