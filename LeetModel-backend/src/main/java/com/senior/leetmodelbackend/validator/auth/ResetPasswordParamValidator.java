...
@Component
public class ResetPasswordParamValidator implements ParamValidator<ResetPasswordDTO> {

    private static final Pattern EMAIL_PATTERN =
            Pattern.compile("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$");

    @Override
    public void validate(ResetPasswordDTO request) {
        ParameterValidator.init()
                ...
                .hasLength(request.getCode(), "验证码不能为空")
                .hasLength(request.getPassword(), "密码不能为空")
                .isTrue(request.getPassword() != null && request.getPassword().length() >= 6, "密码长度不能少于6位")
                .validateAndThrow();
    }
