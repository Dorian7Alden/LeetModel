...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='评审服务消费 Inbox';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='评审服务可靠消息 Outbox';

ALTER TABLE `review_task`
  ADD COLUMN `priority` INT NOT NULL DEFAULT 100 AFTER `status`,
  ADD COLUMN `trace_id` VARCHAR(100) NOT NULL DEFAULT 'migration' AFTER `priority`,
  ADD COLUMN `max_attempts` INT NOT NULL DEFAULT 3 AFTER `attempt_no`,
  ADD COLUMN `lease_owner` VARCHAR(160) NULL AFTER `max_attempts`,
  ...
  ADD INDEX `idx_review_task_claim` (`status`, `next_run_at`, `lease_expires_at`, `priority`, `create_time`),
  ADD INDEX `idx_review_task_lease` (`lease_owner`, `lease_token`, `lease_expires_at`);

UPDATE `review_task`
SET `ai_idempotency_key` = CONCAT('review:', `id`, ':', `workflow_version`, ':attempt:', `attempt_no`)
