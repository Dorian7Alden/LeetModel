...
public class ReviewResultPersistenceService {
    ...
    public ReviewResultPersistenceService(ReviewTaskMapper taskMapper, ReviewV1ResultMapper resultMapper,
                                          ...
                                          MessageEnvelopeFactory envelopeFactory,
                                          MessageOutbox messageOutbox) {
        this(taskMapper, resultMapper, v2ResultMapper, null, null, envelopeFactory, messageOutbox);
    }
    /**
    ...
    public void complete(ReviewTask task, SubmissionReviewDTO submission,
        ...
        if (DeepEvidenceReviewV4Workflow.VERSION_CODE.equals(task.getWorkflowVersion())) {
            completeV4(task, submission, workflowResult);
        } else if (DeepEvidenceReviewV3Workflow.VERSION_CODE.equals(task.getWorkflowVersion())) {
    ...
    private void completeV4(
            ReviewTask task,
            SubmissionReviewDTO submission,
            ReviewWorkflowResult workflowResult
    ) {
        if (workflowResult.parseArtifactId() == null) {
            throw new IllegalArgumentException("V4 评审结果缺少解析产物引用");
        }
        if (v4ResultMapper == null) {
            throw new IllegalStateException("v4ResultMapper 未配置");
        }
        ReviewV4Result result = new ReviewV4Result();
        ...
        result.setParseArtifactId(workflowResult.parseArtifactId());
        result.setWorkflowVersion(task.getWorkflowVersion());
        result.setResultSchemaVersion(DeepEvidenceReviewV4Workflow.RESULT_SCHEMA_VERSION);
        result.setScoringRuleVersion(DeepEvidenceReviewV4Workflow.SCORING_RULE_VERSION);
        result.setScore(workflowResult.score());
        result.setResultJson(workflowResult.resultJson());
        result.setModelName(workflowResult.modelName());
        result.setAiCallId(workflowResult.aiCallId());
        v4ResultMapper.insert(result);
    }
}
