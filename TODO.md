...
- [ ] 任务卡 1: 切片结构化与面包屑上下文增强（Breadcrumbs Injection）
  - 业务流程与职责：在知识切片生成阶段，自动提取所属目录相对路径与文档标题，将语义面包屑（如 `[目录: 数学建模 > 题型方法 > 优化模型] [文档: 线性规划]`）注入到切片头部，增强 Dense 向量与 BM25 在原子短文本上的语义空间坐标。
  - 数据归属与接口：扩展 `KnowledgeChunk` 与 ES 索引 mapping 元数据，不改变对外检索契约，下游无感知提升召回准确率。
  - 实施步骤：
    1. 修改分块逻辑，支持自动注入目录层级与文档标题前缀；
    2. 更新 `RagIdentityFactory` / ES 索引写入元数据；
    3. 编写单测验证分块面包屑注入正确性；
    4. 执行全量构建与检索对比测试。
  - 非目标：暂不改动远程 MinIO 存储（在后续任务卡推进）。
