...
让管理员在内容中心按真实文件夹层级浏览知识库，清楚看到父子目录、当前目录文件名、标签、权威等级与 Token 等有效信息，不再把目录平铺成无层级列表。

入口：

管理端 `/admin/content?view=knowledge` 知识库工作面，以及 `GET /admin/knowledge/tree` 目录树接口。

主流程：

1. 后端依据 Manifest 目录 `path` 组装稳定的父子树，补齐没有独立 Manifest 的中间目录，并聚合后代文档数。
2. 前端以可展开、可折叠的目录树展示父子关系，默认展开当前路径；选择目录后在右侧高密度表格展示该目录的文件名、标题、标签/方法、权威等级和 Token。
3. 搜索同时匹配目录、文件名、标题、标签和方法；匹配结果保留完整祖先链并自动展开，清空后恢复完整目录。
4. 虚拟父目录、无直接文档、深层目录、重复目录名、空结果、刷新失败和 1024px PC 宽度均保持可理解、可操作。

完成标准：

- 接口返回真正嵌套的 `children`，路径与文档归属不丢失，父目录聚合数量正确，并有自动化测试覆盖无序输入、虚拟父目录和重复名称。
- 页面目录节点具备明确的展开/折叠、层级缩进、选中和键盘焦点反馈；文件信息无需打开详情即可辨认。
- 搜索命中子目录或文件时保留祖先层级，不产生新的虚构目录关系；接口异常仍遵守已有真实状态边界。
- 后端目标测试、前端生产构建和真实浏览器主路径验收通过。

修改范围：

- `knowledge-retrieval-service` 的知识树组装与测试。
- 管理端知识库工作面目录树、搜索、文件表格及相关样式。
- 知识库管理 UI/UX 正式设计文档。

非目标：

- 不增加目录或文件的拖拽、移动、重命名、删除和在线正文编辑。
- 不改变 `rag_kb/` Manifest 格式、知识正文事实源、导入导出与索引重建语义。
- 不设计手机端。
