...
- [~] **TASK-REVIEW-PARSE-V2**：PDF解析V2版本功能设计

### 任务卡：TASK-REVIEW-PARSE-V2

- **目标**：完成第二代 PDF 解析（PAPER_PARSE_V2 及产物 Schema PAPER_DOCUMENT_V2）的系统功能设计，建立“双页滑窗提取 + 前文上下文参考 + 重叠页冲突仲裁 + 全局连贯平铺”的极简实用设计体系。
- **入口**：`docs/project/03-微服务设计/ai-review-service/PDF解析V2/` 与对应服务说明。
- **阶段执行路线图**：
  - [x] **步骤 1：全景场景梳理**（已完成 `01-数模论文场景全景.md`，收敛通用学术论文排版要素）。
  - [x] **步骤 2：核心矛盾推导**（已完成 `02-问题清单.md`，推导八大工程矛盾）。
  - [x] **步骤 3：核心假设剪枝**（已完成 `03-假设前提.md`，确立物理页权威、外框页眉页脚忽略、大致页号与忠实提取七大公理）。
  - [x] **步骤 4：解决方案粗粒度设计**（已完成 `04-解决方案设计.md`，确立滑窗调度、HTML表格、整块代码、美观量化等宏观思路）。
  - [x] **步骤 5：端到端执行流程与架构梳理**（已完成 `05-执行流程与架构梳理.md`，理清组件拓扑、五阶段流水线、运行时时序与异常降级）。
  - [x] **步骤 6：数据契约与产物Schema设计**（已完成 `06-数据契约与产物Schema设计.md`，定义极简元素类型、HTML表格载荷、整块代码与粗粒度页码映射）。
  - [x] **步骤 7：降级容错与版本兼容设计**（已完成 `07-降级容错与版本兼容设计.md`，确立 Schema 不可变降级、本地 OCR 兜底与 V1/V2 物理隔离路由）。
  - [x] **步骤 8：提示词体系与输出规范**（已完成 `08-提示词设计与输出规范.md`，确立 Role+Task+Do/Dont 结构、[PLACEHOLDER] 模板、反斜杠双重转义与标准 JSON 契约）。
  - [x] **步骤 9：上下文管理与滑窗调度机制**（已完成 `09-上下文管理与滑窗调度.md`，确立 1-2, 2-3 滑窗步进、前文双重参考注入与断点恢复算法）。
  - [x] **步骤 10：重叠冲突检测与仲裁机制**（已完成 `10-重叠冲突检测与仲裁机制.md`，确立插图解耦均值融合、本地纯文本客观参考与单次仲裁时序）。
- **完成标准**：
  1. `PDF解析V2/` 目录下形成完整、连贯、精简且无过度设计的原子设计文档集。
  2. 经用户逐项交互审核确认，完成 V2 解析功能设计的全量闭环。
- **修改范围**：
  - `TODO.md`
  - `docs/project/03-微服务设计/ai-review-service/PDF解析V1/`
  - `docs/project/03-微服务设计/ai-review-service/PDF解析V2/`
  - `docs/project/03-微服务设计/ai-review-service/README.md`
- **非目标**：
  - 本阶段不修改 Java 业务代码或执行代码级重构（未获明确授权不进入实现阶段）。
  - 不做微观物理行号精确定位，不增加过细的数据结构约束。
  - 不引入特定赛事的承诺书、编号页等定制化判读逻辑。
