...
- **阶段执行路线图**：
  ...
  - [ ] **步骤 8：提示词体系与输出规范**（编写 `08-提示词设计与输出规范.md`，设计双页输入、外框页眉页脚忽略、HTML表格输出与长图描述Prompt）。
  - [ ] **步骤 9：上下文管理与滑窗调度机制**（编写 `09-上下文管理与滑窗调度.md`，深化 1-2, 2-3 滑动步长推进、前文大纲与尾部参考注入机制）。
  - [ ] **步骤 10：重叠冲突检测与仲裁机制**（编写 `10-重叠冲突检测与仲裁机制.md`，细化 PDFBox 本地文本参考与仲裁 AI 决断接口）。
- **完成标准**：
