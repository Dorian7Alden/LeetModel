...
目标：队伍成员可以将 20MB 以内的论文 PDF 按分片上传，在网络中断或页面刷新后重新选择同一文件并只补传缺失分片，最终幂等生成唯一提交版本并触发 AI 评审。

入口：队伍详情页的论文提交区域和 submission-service 的 `/api/submissions/uploads` 上传会话接口。

主流程：前端计算文件摘要并初始化上传会话，查询已接收分片后补传缺失部分；submission-service 校验队伍权限、练习时间和分片完整性，在 MinIO 合并并复核 PDF 后创建提交版本，幂等触发评审并清理临时分片。

完成标准：成功上传、中断续传、重复分片、缺失分片、并发完成、越权、超时和过期清理均有可验证结果；重复完成不产生额外提交或评审任务；submission-service 目标测试、前端生产构建和真实入口联调通过。

修改范围：`submission-service` 的上传会话、Flyway SQL、MinIO 分片存储适配、错误码和测试；队伍详情页与提交 API；对应的论文提交设计文档。

非目标：不实现文件去重和秒传，不放宽论文 PDF 的 20MB 上限，不改造题目附件上传，不将题目附件的大小规则纳入 submission-service。
